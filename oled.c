...
void oledWriteAlarmIcon (unsigned char page, unsigned char column){
    int i, j;
    WriteCommand(page);
    column += OFFSET;
...
void oledWriteCharRaw3x(char letter, unsigned char page, unsigned char column)
{
    int i, shifter, j;
    unsigned char line; 
    BYTE pixels2Print, twoPixels;

    shifter = 0x03;
    letter -= ' ';
    line = page;
    
    for (j = 0; j < 5; j++) /* print the pixels per character */
    {
		if (j == 1)
			column += 1;
        pixels2Print = g_pucFont[letter][j];
        column += OFFSET;
        WriteCommand(0x00+(column&0x0F));
        WriteCommand(0x10+((column>>4)&0x0F));
        for (shifter = 0x03, line = page; shifter < 0xC1; shifter = shifter << 2, line++) /* vertical expantion */
        {
            WriteCommand(line);
            twoPixels = pixels2Print & shifter;
            for (i = 0; i < 3; i++) /* horizintal expantion */
            {
                switch (twoPixels)
                {
                    case 0:
...
void oledPutBigChar(unsigned char *ptr, unsigned char page, unsigned char col){
    BYTE temp;
    int i, j, offset;

    offset = 16;
    page = page + 0xB0;
    oledWriteCharRaw3x(*ptr, page, col);
    while(*++ptr)
    {
        col += offset;
        oledWriteCharRaw3x(*ptr, page, col);
...
ROM BYTE g_pucIcon[4][6] = 
{
    {0xE5, 0x11, 0x09, 0x04, 0x02, 0xFB},
    {0xFB, 0x02, 0x04, 0x09, 0x11, 0xE5},
    {0x09, 0x08, 0x90, 0xE0, 0xC0, 0xCD},
    {0xCD, 0xC0, 0xE0, 0x90, 0x08, 0x09},
};
...
ROM BYTE g_pucFont[95][5] =
{
    ...
    // { 0x99, 0x24, 0x5A, 0x24, 0x99 }, // alarm icon   0X7E
